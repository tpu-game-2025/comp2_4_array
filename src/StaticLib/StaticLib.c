...
void resize(my_array* ar, int n)	// ToDo:配列の要素数を変更しよう！(reallocは禁止)
{
	...
	int copy_num = (ar->num < n) ? ar->num : n;
...
bool set(my_array* ar, int index, int val)	// ToDo:配列の要素を変更しよう！
{
...
int get(const my_array* ar, int index)	// ToDo:要素を所得して、indexがおかしかったら0を返そう
{
...
// my_array の要素数を取得する
int size(const my_array* ar)	// ToDo: 配列の要素数を返そう
{
	if (ar == NULL) return 0;
	return ar->num;
}